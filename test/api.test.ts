...
describe("Cloudflare-native API proxy", () => {
  it("converts chat completions requests into Responses requests", () => {
    ...
    const upstream = JSON.parse(new TextDecoder().decode(prepared.body)) as Record<string, unknown>;
    expect(prepared).toMatchObject({ kind: "chat", model: "gpt-5.6-sol", streaming: true, method: "POST" });
  ...
  it("preserves model-suffix reasoning effort for native Responses requests", () => {
    ...
    const upstream = JSON.parse(new TextDecoder().decode(prepared.body)) as Record<string, unknown>;
    expect(prepared).toMatchObject({ kind: "responses", model: "gpt-5.6-terra", streaming: true });
    expect(upstream).toMatchObject({ model: "gpt-5.6-terra", reasoning: { effort: "max", summary: "auto" } });
  });

  it("transforms upstream Responses events into streaming chat chunks", async () => {
    const transformed = transformChatStream(stream(upstreamEvents().slice(0, 80), upstreamEvents().slice(80)), "gpt-5.6-sol");
    ...
    expect(chunks.some((chunk) => JSON.stringify(chunk).includes('"content":"hello"'))).toBe(true);
    expect(chunks.some((chunk) => JSON.stringify(chunk).includes('"content":" world"'))).toBe(true);
  ...
  it("buffers upstream events for non-streaming chat and Responses clients", async () => {
    const chat = await bufferChatCompletion(stream(upstreamEvents()), "gpt-5.6-sol");
    ...
    const responses = await bufferResponses(stream(
    ...
    expect(responses).toMatchObject({ id: "resp_1", output: [{ type: "message" }], usage: { total_tokens: 4 } });
  });

  it("converts the live entitlement catalog into OpenAI-compatible model entries", () => {
    expect(modelsFromUpstream({ models: [{
      slug: "gpt-5.6-sol",
      ...
      object: "list",
      data: [
        expect.objectContaining({ id: "gpt-5.6-sol", base_model: "gpt-5.6-sol" }),
        expect.objectContaining({ id: "gpt-5.6-sol-low", reasoning_effort: "low" }),
        expect.objectContaining({ id: "gpt-5.6-sol-max", reasoning_effort: "max" }),
      ],
    });
  });
});
