...
export function prepareProxyRequest(pathname: string, bodyBytes?: ArrayBuffer): PreparedProxyRequest {
...
export function upstreamHeaders(request: Request, account: UpstreamAccount, kind: PreparedProxyRequest["kind"]): Headers {
  ...
  if (kind !== "models") {
    ...
    headers.set("session_id", request.headers.get("session_id") || request.headers.get("session-id") || crypto.randomUUID());
    headers.set("x-codex-beta-features", request.headers.get("x-codex-beta-features") || "multi_agent,apps,prevent_idle_sleep");
...
export async function finalizeUpstreamResponse(
  ...
  if (prepared.streaming) {
    return new Response(transformChatStream(requiredBody(response), prepared.model), {
      status: response.status,
      headers: streamingHeaders(response.headers),
    });
  }
  return Response.json(await bufferChatCompletion(requiredBody(response), prepared.model));
}
...
export function modelsFromUpstream(input: unknown): JsonObject {
  ...
  for (const value of models) {
    ...
    if (!slug || stringValue(model?.visibility) !== "list") continue;
    const displayName = stringValue(model?.display_name) || slug;
    data.push(modelEntry(slug, displayName, slug));
    ...
    for (const levelValue of levels) {
      const effort = stringValue(objectValue(levelValue)?.effort);
      if (!effort || effort === "ultra") continue;
      data.push(modelEntry(`${slug}-${effort}`, `${displayName} (${effort} reasoning)`, slug, effort));
    }
  }
  if (!data.length) throw new PoolError(502, "Upstream returned no usable models");
  return { object: "list", data };
}

export function transformChatStream(body: ReadableStream<Uint8Array>, model: string): ReadableStream<Uint8Array> {
  const parser = new SseParser();
  const decoder = new TextDecoder();
  const transformer = new ChatEventTransformer(model);
  return body.pipeThrough(new TransformStream<Uint8Array, Uint8Array>({
    transform(chunk, controller) {
      emitChatPayloads(parser.push(decoder.decode(chunk, { stream: true })), transformer, controller);
    },
    flush(controller) {
      emitChatPayloads(parser.push(decoder.decode(), true), transformer, controller);
      controller.enqueue(encoder.encode("data: [DONE]\n\n"));
    },
  }));
}

export async function bufferResponses(body: ReadableStream<Uint8Array> | null): Promise<JsonObject> {
  ...
  for await (const payload of readSse(body)) {
    ...
    if (type === "response.output_item.done") {
      ...
      const message = stringValue(objectValue(event.error)?.message) || "Upstream Responses stream returned an error";
      throw new PoolError(502, message);
    }
  }
  if (!completed) throw new PoolError(502, "Responses stream ended without a terminal response");
  if (output.size) completed.output = [...output.entries()].sort(([left], [right]) => left - right).map(([, item]) => item);
  return completed;
}

export async function bufferChatCompletion(body: ReadableStream<Uint8Array>, model: string): Promise<JsonObject> {
  const transformer = new ChatEventTransformer(model);
  let responseId = "";
  ...
  for await (const payload of readSse(body)) {
    for (const chunk of transformer.transform(payload)) {
      ...
      const choice = objectValue(Array.isArray(chunk.choices) ? chunk.choices[0] : undefined);
      ...
      retainedBytes += encoder.encode(text).byteLength + encoder.encode(reasoning).byteLength;
      if (retainedBytes > MAX_BUFFERED_RESPONSE_BYTES) throw new PoolError(502, "Buffered response is too large");
      const finish = stringValue(choice?.finish_reason);
      if (finish) finishReason = finish;
      const deltas = Array.isArray(delta?.tool_calls) ? delta.tool_calls : [];
      for (const callDeltaValue of deltas) mergeToolCall(toolCalls, objectValue(callDeltaValue));
    }
  }
  const message: JsonObject = { role, content: content || (toolCalls.size ? null : "") };
...
function prepareChatRequest(body: JsonObject): PreparedProxyRequest {
  ...
  for (const value of messages) {
    ...
    if (role === "assistant") {
      ...
      for (const callValue of calls) {
        ...
        input.push({ type: "function_call", call_id: callId, name, arguments: argumentsValue(fn?.arguments) });
      }
      continue;
    }
    if (role === "tool") {
      const callId = stringValue(message.tool_call_id);
      if (callId) input.push({ type: "function_call_output", call_id: callId, output: collectText(message.content) });
    }
  }
  const upstream: JsonObject = {
    ...
    parallel_tool_calls: typeof body.parallel_tool_calls === "boolean" ? body.parallel_tool_calls : false,
    reasoning: reasoningSettings(body, model),
    include: ["reasoning.encrypted_content"],
    store: false,
    stream: true,
  };
  if (typeof body.prompt_cache_key === "string") upstream.prompt_cache_key = body.prompt_cache_key;
  return {
    kind: "chat",
    model,
...
function normalizeResponsesInput(body: JsonObject): void {
  ...
  if (instructions && systemText) filtered.unshift({ role: "developer", content: systemText });
...
function normalizeModel(input: string): string {
  ...
  for (const effort of [...REASONING_EFFORTS].sort((left, right) => right.length - left.length)) {
...
function reasoningSettings(body: JsonObject, model: string): JsonObject {
  ...
  let effort = stringValue(existing?.effort) || stringValue(body.reasoning_effort);
  if (!effort) {
    const requestedModel = stringValue(body.model);
    effort = REASONING_EFFORTS.find((candidate) => requestedModel.endsWith(`-${candidate}`)) || "";
  }
  if (!REASONING_EFFORTS.includes(effort as typeof REASONING_EFFORTS[number])) {
    effort = model.includes("5.6") || model.includes("daybreak") ? "low" : "medium";
  }
...
function userContent(value: unknown): JsonObject[] {
  ...
  for (const partValue of value) {
    ...
    if (type === "text" || type === "input_text") {
      ...
      if (url) result.push({ type: "input_image", image_url: url, ...(image?.detail ? { detail: image.detail } : {}) });
...
class SseParser {
  ...
  push(text: string, final = false): string[] {
    ...
    if (this.pending.length > MAX_SSE_EVENT_BYTES) throw new PoolError(502, "Upstream SSE event is too large");
    if (final) {
  ...
  private consumeLine(rawLine: string, payloads: string[]): void {
    ...
    if (this.eventBytes > MAX_SSE_EVENT_BYTES) throw new PoolError(502, "Upstream SSE event is too large");
...
class ChatEventTransformer {
  ...
  transform(payload: string): JsonObject[] {
    ...
    if (type.startsWith("response.reasoning") && type.endsWith(".delta")) {
      ...
      return text ? [...this.roleChunk(), this.chunk({ reasoning_content: text })] : [];
    }
    if (type === "response.output_text.delta") {
      return [...this.roleChunk(), this.chunk({ content: stringValue(event.delta) })];
    }
    if (type === "response.output_item.added") return this.toolStart(event);
    if (type === "response.function_call_arguments.delta") return this.toolArguments(event);
  ...
  private toolStart(event: JsonObject): JsonObject[] {
    ...
    return [...this.roleChunk(), this.chunk({
      tool_calls: [{
        index,
  ...
  private toolArguments(event: JsonObject): JsonObject[] {
    ...
    return [...this.roleChunk(), this.chunk({
      tool_calls: [{ index, function: { arguments: stringValue(event.delta) } }],
    })];
  }

  private chunk(delta: JsonObject, finishReason: string | null = null, usage?: JsonObject): JsonObject {
    return {
      id: this.responseId,
...
function mergeToolCall(calls: Map<number, JsonObject>, delta: JsonObject | undefined): void {
  ...
  current.function = {
    ...
    arguments: stringValue(currentFunction.arguments) + stringValue(deltaFunction?.arguments),
  };
  calls.set(index, current);
}

function chatUsage(usage: JsonObject | undefined): JsonObject {
  const promptTokens = numberValue(usage?.input_tokens) ?? numberValue(usage?.prompt_tokens) ?? 0;
  const completionTokens = numberValue(usage?.output_tokens) ?? numberValue(usage?.completion_tokens) ?? 0;
  ...
  return {
    ...
    total_tokens: numberValue(usage?.total_tokens) ?? promptTokens + completionTokens,
...
function modelEntry(id: string, name: string, baseModel: string, effort?: string): JsonObject {
  return {
    id,
    object: "model",
    created: 0,
