...
import { readTokenUsage, requestMetadata } from "../src/metrics";
...
describe("request metrics", () => {
  it("extracts safe request metadata without retaining the request payload", () => {
    const body = encoder.encode(JSON.stringify({ model: "gpt-5.6-sol", stream: true, input: "private prompt" })).buffer as ArrayBuffer;
    expect(requestMetadata("/v1/responses", body)).toEqual({
      model: "gpt-5.6-sol",
      endpoint: "/v1/responses",
      streaming: true,
    });
    expect(requestMetadata("/v1/models")).toEqual({
      model: "model-catalog",
      endpoint: "/v1/models",
      streaming: false,
    });
